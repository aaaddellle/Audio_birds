...
class MinimagenCollator:
    ...
    def __call__(self, batch):
        ...
        batch = [x for x in batch if isinstance(x, dict) and x.get('image') is not None]

        if not batch:
            return None
        
        
        # Convert images to tensors and move to the specified device
        for didx, datum in enumerate(batch):
            for key, value in datum.items():
                if isinstance(value, Image.Image):  # Checking if the object is a PIL Image
                    datum[key] = to_tensor(value).to(self.device)  # Convert and move to device
                elif isinstance(value, torch.Tensor):  # If already a tensor, just move to device
                    datum[key] = value.to(self.device)
...
@contextmanager
def training_dir(dir_name):
    original_dir = os.getcwd()
    new_dir = os.path.join(original_dir, dir_name)
    if not os.path.exists(new_dir):
        os.makedirs(new_dir)
        logging.info(f"Directory {new_dir} created.")
    os.chdir(new_dir)
    logging.debug(f"Changed working directory to {new_dir}")
    try:
        yield
    finally:
        os.chdir(original_dir)
        logging.debug(f"Reverted to original directory {original_dir}")
...
class MinimagenDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        enc, msk = t5_encode_text.predict([self.captions[idx]])
...
'''def load_custom_dataset(data_path, split_ratio=0.8, valid_num=None):
    """
    Load custom dataset from a local directory and divide it into train and validation sets.

    :param data_path: Path to the directory containing the custom dataset.
    :param split_ratio: Ratio to split the data into training and validation sets (default is 0.8).
    :param valid_num: Number of samples to include in the validation set (optional).
    :return: train_dataset, valid_dataset
    """
    # Load your own data using the load_data function
    x_data, y_data = load_owndata(data_path)
    ...
    x_train, x_valid = x_data[:split_index], x_data[split_index:]
    y_train, y_valid = y_data[:split_index], y_data[split_index:]

    # Optionally limit the number of samples in the validation set
    if valid_num is not None:
        x_valid, y_valid = x_valid[:valid_num], y_valid[:valid_num]
...
'''def ConceptualCaptions(args, smalldata=False, testset=False):
    ...
    if testset:
        ...
        return train_dataset, valid_dataset'''
def load_audio_to_image_dataset(data_path, transform, split_ratio=0.8, testset=False):
    """
    ...
    if testset:
        ...
        test_dataset = AudioToImageDataset(features_path=f"{data_path}/test_features.json", 
                                           images_dir=f"{data_path}/test_images", 
                                           transform=transform)
        return test_dataset
    else:
...
def MinimagenTrain(timestamp, args, unets, imagen, train_dataloader, valid_dataloader, training_dir, optimizer,
    ...
    def train():
        images = batch['image']
        audio_embeds = batch['feature']

        losses = [0. for i in range(len(unets))]
        for unet_idx in range(len(unets)):
            loss = imagen(images, audio_embeds = audio_embeds, unet_number=unet_idx + 1)
        ...
        if batch_num % args.CHCKPT_NUM == 0:
            ...
            try:
                with training_dir("tmp"):
                    ...
                    logging.debug(f"Saving model state to {model_path} in directory {os.getcwd()}")
                    torch.save(imagen.unets[idx].state_dict(), model_path)
                    logging.info(f"Model saved successfully at {model_path}")
            except Exception as e:
                logging.error(f"Failed to save model at {model_path}: {str(e)}")
                raise
            '''with training_dir("tmp"):
            ...
            for vbatch in tqdm(valid_dataloader):
                ...
                images = vbatch['image']
                audio_embeds = vbatch['feature']

                for unet_idx in range(len(unets)):
                    running_valid_loss[unet_idx] += imagen(images, audio_embeds = audio_embeds,
                ...
                if l < best_loss[i]:
                    ...
                    try:
                        with training_dir("state_dicts"):
                            model_path = f"unet_{i}_state_dict.pth"
                            logging.debug(f"Saving model state to {model_path} in directory {os.getcwd()}")
