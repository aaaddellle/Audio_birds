...
class Imagen(nn.Module):
    ...
    def _p_sample(self,
        ...
        audio_embeds = audio_embeds.repeat(b, 1) if audio_embeds.dim() == 1 else audio_embeds
        
        model_mean, _, model_log_variance = self._p_mean_variance(unet, x=x, t=t,
                                                                  noise_scheduler=noise_scheduler,
                                                                  audio_embeds=audio_embeds *cond_scale, 
                                                                  #text_mask=text_mask,
                                                                  cond_scale=cond_scale,
                                                                  lowres_cond_img=lowres_cond_img,
                                                                  lowres_noise_times=lowres_noise_times)
    ...
    def _p_sample_loop(self,
        ...
        #  reverse-diffusion timesteps (i.e. (T, T-1, T-2, ..., 2, 1, 0) )
        batch = shape[0]
    ...
    def sample(
        ...
        '''if exists(texts) and not exists(text_embeds):
            text_embeds = t5_encode_text.predict(texts)
            text_embeds = map(lambda t: t.to(device), (text_embeds))'''

        assert audio_embeds.shape[-1] == self.audio_embed_dim, f'invalid audio embedding dimension passed in (should be {self.audio_embed_dim})'


        assert exists(audio_embeds), 'text or text encodings must be passed into Imagen'
        assert not (exists(audio_embeds) and audio_embeds.shape[
            -1] != self.audio_embed_dim), f'invalid text embedding dimension being passed in (should be {self.text_embed_dim})'
    ...
    def forward(
        ...
        '''if exists(texts) and not exists(audio_embeds):
            assert len(texts) == len(images), \
                'number of text captions does not match up with the number of images given'

            audio_embeds = t5_encode_text.predict(texts)
            audio_embeds = map(lambda t: t.to(images.device), (audio_embeds))'''
        
        print(audio_embeds.shape)  # Should output something like (batch_size, 128)
        ...
        assert not (exists(audio_embeds) and audio_embeds.shape[-1] != self.audio_embed_dim), \
            f'invalid text embedding dimension being passed in (should be {self.audio_embed_dim})'
        ...
        return self._p_losses(unet, images, times, audio_embeds=audio_embeds, 
                              #text_mask=text_masks,
