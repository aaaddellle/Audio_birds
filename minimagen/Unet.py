...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        self.audio_to_cond = nn.Linear(self.audio_embed_dim, cond_dim)
        ...
        #   https://www.assemblyai.com/blog/how-imagen-actually-works/#classifier-free-guidance
         # For classifier-free guidance, create null tokens; adapt to use with audio
        self.null_audio_embed = nn.Parameter(torch.randn(1, 1, cond_dim))  # Assuming audio embedding needs only one null token
        self.null_audio_hidden = nn.Parameter(torch.randn(1, time_cond_dim))
    ...
    def _cast_model_parameters(
        ...
        if lowres_cond == self.lowres_cond and \
                ...
                audio_embed_dim == self.audio_embed_dim and \
    ...
    def forward(
        ...
        # text conditioning
        t, c = self._audio_condition(audio_embeds, batch_size, cond_drop_prob, device, t, time_tokens)
    ...
    def _audio_condition(
        ...
        audio_tokens = self.norm_cond(audio_embeds)
        
        # Project the audio embeddings to the conditioning dimension
        audio_tokens = self.audio_to_cond(audio_tokens)

        # Prob. mask for classifier-free guidance conditional dropout. Tells which elements in the batch to keep.
        audio_keep_mask = F.dropout(torch.ones((batch_size, 1, 1), device=device), p=cond_drop_prob, training=True)

        # Create NULL tensor for classifier-free guidance, matches the dimension of the audio tokens
        null_audio_embed = self.null_audio_embed.expand(batch_size, -1, -1).to(audio_tokens.dtype)

        # Replace dropped elements with NULL values
        audio_tokens = torch.where(audio_keep_mask.bool(), audio_tokens, null_audio_embed)

        # Extra non-attention conditioning by projecting and then summing audio embeddings to time conditioning
        mean_pooled_audio_tokens = audio_tokens.mean(dim=-2)  # Pool along the embedding dimension

        # Project to `time_cond_dim`
        audio_hiddens = self.to_audio_non_attn_cond(mean_pooled_audio_tokens)  # (b, cond_dim) -> (b, time_cond_dim)

        # Drop relevant conditioning info as demanded by classifier-free guidance mask
        audio_hiddens = torch.where(audio_keep_mask.bool(), audio_hiddens, self.null_audio_hidden.to(t.dtype))

        # Add this conditioning to our `t` tensor
        t = t + audio_hiddens

        # main conditioning tokens `c` - here, simply use the processed audio tokens
        c = audio_tokens
